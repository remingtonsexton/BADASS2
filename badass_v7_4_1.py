...
def outflow_test_oiii(lam_gal,galaxy,noise,run_dir,fwhm_gal,velscale,outflow_test_niter,outflow_test_pars,print_output=True):
	...
	if ((med_resid_std_no_outflow-med_resid_std_outflow)/(med_resid_std_err_no_outflow+med_resid_std_err_outflow))>=(outflow_test_pars['resid_test']['nsigma']):
	...
	voff_metric =  np.abs( (pdict['na_oiii5007_core_voff']['med'])-(pdict['na_oiii5007_outflow_voff']['med']) )/np.sqrt( (pdict['na_oiii5007_core_voff']['std'])**2 + (pdict['na_oiii5007_outflow_voff']['std'])**2 )
	...
	write_log((pdict,
			   ...
			   amp_metric,fwhm_metric,voff_metric,resid_metric,\
...
def outflow_test_nii(lam_gal,galaxy,noise,run_dir,fwhm_gal,velscale,outflow_test_niter,outflow_test_pars,print_output=True):
	...
	voff_metric =  np.abs( (pdict['na_Ha_core_voff']['med'])-(pdict['na_Ha_outflow_voff']['med']) )/np.sqrt( (pdict['na_Ha_core_voff']['std'])**2 + (pdict['na_Ha_outflow_voff']['std'])**2 )
	resid_metric = ((med_resid_std_no_outflow-med_resid_std_outflow)/np.sqrt(med_resid_std_err_no_outflow**2+med_resid_std_err_outflow**2))
	# print('\n Amp. Metric = %0.2f' % amp_metric)
	# print(' FWHM Metric = %0.2f' % fwhm_metric)
	# print(' VOFF Metric = %0.2f' % voff_metric)
...
def write_log(output_val,output_type,run_dir):
	...
	if (output_type=='outflow_test'):
		...
		# 	   np.median(resid_std_outflow),np.std(resid_std_outflow),
		# 	   ),'outflow_test',run_dir)
		rdict,sigma,total_resid_noise,total_resid_noise_err,amp_metric,fwhm_metric,voff_metric,resid_metric,resid_std_no_outflow,resid_dstd_no_outflow,resid_std_outflow,resid_dstd_outflow = output_val
		logfile = open(run_dir+'log/log_file.txt','a')
		logfile.write('\n')
