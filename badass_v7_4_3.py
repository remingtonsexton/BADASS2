...
def outflow_test_oiii(lam_gal,galaxy,noise,run_dir,fwhm_gal,velscale,outflow_test_niter,outflow_test_pars,print_output=True):
	...
	for i in range(outflow_test_niter):
		resid_std_outflow.append(np.std(mccomps_outflow['resid'][:,i][(lam_gal >= min_wave) & (lam_gal <= max_wave)]))
		resid_std_no_outflow.append(np.std(mccomps_no_outflow['resid'][:,i][(lam_gal >= min_wave) & (lam_gal <= max_wave)]))
		total_resid_noise.append(np.std(mccomps_outflow['resid'][:,i]))
	...
	med_resid_std_outflow        = np.median(resid_std_outflow)
	med_resid_std_err_outflow    = np.std(resid_std_outflow)
	med_resid_std_no_outflow     = np.median(resid_std_no_outflow)
	med_resid_std_err_no_outflow = np.std(resid_std_no_outflow)
	med_total_resid_noise        = np.median(total_resid_noise)
	med_total_resid_noise_err    = np.std(total_resid_noise)
	resid_metric = ((med_resid_std_no_outflow-med_resid_std_outflow)/np.sqrt(med_resid_std_err_no_outflow**2+med_resid_std_err_outflow**2))
	fstat_metric = (med_resid_std_no_outflow)**2/(med_resid_std_outflow)**2

	# print ((med_resid_std_no_outflow-med_resid_std_outflow)/(med_resid_std_err_no_outflow+med_resid_std_err_outflow)),(outflow_test_pars['resid_test']['nsigma'])
	...
	amp_metric = ((pdict['na_oiii5007_outflow_amp']['med'])/np.sqrt( (pdict['na_oiii5007_outflow_amp']['std'])**2 + (outflow_test_pars['amp_test']['nsigma']*sigma)**2 ))
	...
	if print_output:  
		...
		print('{0:<30}{1:<25.2f}{2:<25.2f}'.format('total resid noise',np.median(med_total_resid_noise),np.std(med_total_resid_noise)))
		...
		print('{0:<30}{1:<25.2f}'.format('resid metric',resid_metric))
		print('{0:<30}{1:<25.2f}'.format('f-stat metric',fstat_metric))
		print('{0:<30}{1:<25.2f}{2:<25.2f}'.format('[OIII] no outflow resid. std.',np.median(resid_std_no_outflow),np.std(resid_std_no_outflow)))
		print('{0:<30}{1:<25.2f}{2:<25.2f}'.format('[OIII] outflow resid. std.',np.median(resid_std_outflow),np.std(resid_std_outflow)))
		print('--------------------------------------------------------------------------------------')

	# Write to log
	write_log((pdict,
			   sigma,np.median(med_total_resid_noise),np.std(med_total_resid_noise),
			   amp_metric,fwhm_metric,voff_metric,resid_metric,fstat_metric,\
			   np.median(resid_std_no_outflow),np.std(resid_std_no_outflow),
			   np.median(resid_std_outflow),np.std(resid_std_outflow),
	...
	if (outflow_test_pars['resid_test']['test']==True):
		resid_cond = (resid_metric>(outflow_test_pars['resid_test']['nsigma']))
		if (resid_cond==True):
			outflow_conds.append(True)
	...
	#   (5) f-statistic test (ratio of variances):
	if (outflow_test_pars['resid_test']['test']==True):
		fstat_cond = (fstat_metric>(outflow_test_pars['fstat_test']['nsigma']))
		if (fstat_cond==True):
			outflow_conds.append(True)
			outflow_test_pars['fstat_test']['pass'] = True
			if print_output:
				print('{0:<35}{1:<30}'.format('f-statistic condition:', 'Pass' ) )
		elif (fstat_cond==False):
			outflow_conds.append(False)
			outflow_test_pars['fstat_test']['pass'] = False
			if print_output:
				print('{0:<35}{1:<30}'.format('f-statistic condition:', 'Fail' ) )

	#	(6) Parameter bounds: if paramter reached the parameter limits (bounds), then its uncertainty is zero, and something went wrong with the fit.
...
def outflow_test_nii(lam_gal,galaxy,noise,run_dir,fwhm_gal,velscale,outflow_test_niter,outflow_test_pars,print_output=True):
	...
	# Outflow vs. No-outflow Model Metrics; these metrics test significance of parameters between models, and are printed to logfile 
	# regardless of whether they are tested or not.
	amp_metric = ((pdict['na_Ha_outflow_amp']['med'])/np.sqrt( (pdict['na_Ha_outflow_amp']['std'])**2 + (outflow_test_pars['amp_test']['nsigma']*sigma)**2 ))
...
def write_log(output_val,output_type,run_dir):
	...
	if (output_type=='outflow_test'):
		...
		# 	   np.median(resid_std_outflow),np.std(resid_std_outflow),
		# 	   ),'outflow_test',run_dir)
		rdict,sigma,total_resid_noise,total_resid_noise_err,amp_metric,fwhm_metric,voff_metric,resid_metric,fstat_metric,resid_std_no_outflow,resid_dstd_no_outflow,resid_std_outflow,resid_dstd_outflow = output_val
		logfile = open(run_dir+'log/log_file.txt','a')
		logfile.write('\n')
		...
		logfile.write('\n{0:<30}{1:<30.4f}{2:<30.4f}'.format('total resid. noise',total_resid_noise,total_resid_noise_err))
		...
		logfile.write('\n{0:<30}{1:<30.4f}'.format('resid metric',resid_metric))
		logfile.write('\n{0:<30}{1:<30.4f}'.format('f-stat metric',fstat_metric))
	...
	if (output_type=='outflow_test_pass'):
		...
		if (outflow_test_pars['fstat_test']['test']==True):
			logfile.write('\n{0:<35}{1:<30}'.format('f-statistic condition:'	     , str(bool(outflow_test_pars['fstat_test']['pass'])) ))
		if (outflow_test_pars['bounds_test']['test']==True):
